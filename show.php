...
<body>
  <?php 
  include('connect.php');
  $query = "SELECT * FROM customer;" or die("Error:" . mysqli_error($con)); 
   
  $result = mysqli_query($con, $query); 
   ?>
    <div class="container">
        <button type="button" class="btn btn-light btn-lg btn-block">Hluas Nkauj Thiab Hluas Nraug Ua Koj Xaiv</button>
          ...
          <?php while($row = mysqli_fetch_array($result)){
            ...
            echo"<div class='card-body'>
                    ...
                    <button type='button' class='btn-close' data-dismiss='modal' aria-label='Close'></button>
                     ...
                     <p class=''>Sijhawm: ".$row['tim']."<p>Zos: ".$row['vilage']."</p>"."<p>Nroog: ".$row['city']."</p>"."<p> Xeev: ".$row['provin']."</p>"."<p> Leb Xyovtooj: ".$row['numberphome']."</p>"."</p>
                     </div>
                    <div class='modal-footer'>
                    <button type='button' class='btn btn-danger' data-dismiss='modal'>Tsis Yuav</button>
                    <a href='http://api.whatsapp.com/send?phone=+85697348433'><button  class='btn btn-info'>Yuav</button></a>
                     </div>
                    </div>
                    </div>
                  </div>
